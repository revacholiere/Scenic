...
class BehaviorAgent(BasicAgent):
    ...
    def _tailgating(self, waypoint, vehicle_list):
        ...
        if behind_vehicle_state and self._speed < behind_vehicle.speed:
            if (
                (
    ...
    def collision_and_car_avoid_manager(self, waypoint):
        ...
        vehicle_list = self._objects
        vehicle_classes = [1, 2, 3, 5, 7]

        def dist(v):
            return v.location.distance(waypoint.transform.location)

        vehicle_list = [v for v in vehicle_list if dist(v) < 45 and v.category in vehicle_classes]
    ...
    def pedestrian_avoid_manager(self, waypoint):
        ...
        walker_list = self._objects

        def dist(w):
            return w.location.distance(waypoint.transform.location)

        walker_list = [w for w in walker_list if dist(w) < 10 and w.category == 0]
    ...
    def run_step(self):
        ...
        if walker_state:
            ...
            distance = (
                w_distance
                - max(walker.bbox_3d.extent.y, walker.bbox_3d.extent.x)
                - max(
            ...
            logger.debug(f"Pedestrian {walker} detected at {distance}m")

            # Emergency brake if the car is very close.
            if distance < self._behavior.braking_distance:
                # logger.debug("Pedestrian too close, emergency stop")
                # print("Pedestrian too close, emergency stop")
                return self.emergency_stop()
        ...
        if vehicle_state:
            ...
            distance = (
                distance
                - max(vehicle.bbox_3d.extent.y, vehicle.bbox_3d.extent.x)
                - max(
                    self._vehicle.bounding_box.extent.y,
                    self._vehicle.bounding_box.extent.x,
                )
            ...
            logger.debug(f"Vehicle {vehicle} detected at {distance}m")

            # Emergency brake if the car is very close.
            if distance < self._behavior.braking_distance:
                logger.debug("Vehicle too close, emergency stop")
                # print(f"Vehicle too close, emergency stop. {distance}")
                return self.emergency_stop()
            else:
                logger.debug("Vehicle close, following")
                # print('Vehicle close, following')
        ...
        # 3: Intersection behavior
            ...
            logger.debug(f"Intersection detected, target speed: {target_speed}")
            self._local_planner.set_speed(target_speed)
            control = self._local_planner.run_step()
        ...
        # 4: Normal behavior
            ...
            logger.debug(f"Normal behavior, target speed: {target_speed}")
            self._local_planner.set_speed(target_speed)
            control = self._local_planner.run_step()
