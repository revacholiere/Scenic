...
def filter_angle_distance(vehicles_list, sensor, max_dist=50):
...
def save_output(carla_img, bboxes, vehicle_class=None, old_bboxes=None, old_vehicle_class=None, cc_rgb=carla.ColorConverter.Raw, path='', save_patched=False, add_data=None, out_format='pickle', count = None):
    carla_img.save_to_disk(path + 'out_rgb/%06d.png' % count, cc_rgb)
    ...
    if out_format=='json':
        filename = path + 'out_bbox/%06d.txt' % count
        if not os.path.exists(os.path.dirname(filename)):
            os.makedirs(os.path.dirname(filename))
        with open(filename, 'w') as outfile:
            json.dump(out_dict, outfile, indent=4)
    else:
        filename = path + 'out_bbox/%06d.pkl' % count
        if not os.path.exists(os.path.dirname(filename)):
            os.makedirs(os.path.dirname(filename))
        with open(filename, 'w') as outfile:
            json.dump(out_dict, outfile, indent=4)     
    ...
    if save_patched:
        ...
        for crop in bboxes:
            ...
            crop_bbox = [(u1,v1),(u2,v2)]
            img_draw.rectangle(crop_bbox, outline ="red")
        filename = path + 'out_rgb_bbox/%06d.png' % count
        if not os.path.exists(os.path.dirname(filename)):
            os.makedirs(os.path.dirname(filename))
        image.save(filename)
